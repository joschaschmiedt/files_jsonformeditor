...
export class SidebarPreview {
  attach(manager) {
    ...
    supportedMimeTypes.forEach((value) =>
  ...
  handlePreview(model, $thumbnailDiv, $thumbnailContainer, fallback) {
    ...
    this.getFileContent(model.getFullPath()).then(
      function (content) {
  ...
  getFileContent(path) {
    ...
    return $.ajax({
      url: OC.generateUrl('/apps/files_jsonformeditor/ajax/loadfile'),
      data: {
        filename: file,
        dir: dir,
      },
