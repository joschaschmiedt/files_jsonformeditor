...
$(document).ready(function () {
  ...
  if (isPublic && mimetype === 'text/markdown' && filesize < 524288) {
    var sharingToken = $('#sharingToken').val()
    var downloadUrl = OC.generateUrl('/s/{token}/download', {
      token: sharingToken,
    })
    var previewElement = $('#imgframe')
    var renderer = new (marked__WEBPACK_IMPORTED_MODULE_0___default().Renderer)()
    ...
    previewElement.addClass('icon-loading').children().remove()
    ...
    $.get(downloadUrl)
      ...
      .fail(function (result) {
        previewElement.removeClass('icon-loading')
      })
  } else if (isPublic && mimetype.substr(0, mimetype.indexOf('/')) === 'text') {
    // Based on default text previews from "files_sharing/js/public.js", but
    // using the public endpoint from files_jsonformeditor for better character
    // encoding support.
    var previewElement = $('#imgframe')
    previewElement.addClass('icon-loading').children().remove()
    ...
    var sharingToken = $('#sharingToken').val()
    $.ajax({
      url: OC.generateUrl('/apps/files_jsonformeditor/public/{token}', {
        token: sharingToken,
      }),
      headers: {
