...
export const JsonFormEditor = {
  ...
  loadEditor: function (container, file) {
    ...
    this.loadFile(
      ...
      function (message) {
        ...
        OC.dialogs.alert(
          message,
          t('files_jsonformeditor', 'An error occurred!')
        )
        _self.closeEditor()
      }
    )
  ...
  loadFile: function (dir, filename, success, failure) {
    fetch(
      generateUrl(
        '/apps/files_jsonformeditor/ajax/loadfile?' +
          new URLSearchParams({
            filename: filename,
            dir: dir,
          })
  ...
  saveFile: function (data, file, success, failure) {
    ...
    fetch(generateUrl('/apps/files_jsonformeditor/ajax/savefile'), {
      method: 'PUT',
      headers: {
  ...
  _onClickDocument: function (event) {
    ...
    if (
      !$(event.target).closest('#editor_container').length &&
